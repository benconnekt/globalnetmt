...
class sms {
    ...
    function send($to=null, $from=null, $text=null) {
        ...
        if ($this->unicode == true) {
            ...
            if (mb_strlen ($text) > 210) {
        	    die ("Your unicode message is to long! (Current lenght=".mb_strlen ($text).")");
        	}
    ...
    */
    function _execgw($command) {
        ...
        display_message ("Unsupported sending method!");
    }
    ...
    */
    function _fopen($command) {
        ...
        if ($handler) {
            ...
            display_message ("Error while executing fopen sending method!<br>Please check does PHP have OpenSSL support and check does PHP version is greater than 4.3.0.");
        }
    }
    ...
    */
    function _parse_auth ($result) {
        ...
        if ($code!="OK") {
            display_message ("Error in SMS authorization! ($result)");
        }
        return $session;
    }
    ...
    */
    function _parse_send ($result) {
    	$code = substr($result, 0, 2);
    	if ($code!="ID") {
    	    display_message ("Error sending SMS! ($result)");
    	} else {
    	    $code = "OK";
    	}
        return $code;
    ...
    */
    function _chk_curl() {
        if (!extension_loaded('curl')) {
            display_message ("This SMS API class can not work without CURL PHP module! Try using fopen sending method.");
        }
    }
    ...
    */
    function _chk_mbstring() {
        if (!extension_loaded('mbstring')) {
            display_message ("Error. This SMS API class is setup to use Multibyte String Functions module - mbstring, but module not found. Please try to set unicode=false in class or install mbstring module into PHP.");
        }
    }
